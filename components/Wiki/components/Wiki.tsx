...
const Wiki = () => {
  ...
  const handleSearch = (value: string) => {
    ...
    let filteredData = dataBase.filter(
      (item) => item.entry.filter((elem) => elem.title.toLowerCase().includes(value.toLowerCase())).length > 0
    );
    ...
    filteredData = filteredData.map((item) => ({
      letter: item.letter,
      entry: item.entry.filter((element) => element.title.toLowerCase().includes(value.toLowerCase())),
    }));
  ...
  const populateData = (entries: wikiEntry[]) => {
    ...
    alphaBet.forEach((item) => {
      helperArray.push({
        ...
        entry: entries.filter((element) => element.title.charAt(0).toUpperCase() === item),
      });
    });
  ...
  useEffect(() => {
    getEntries()
      .then((entries) => sortEntries(entries))
      .then((entries) => populateData(entries));
  }, []);
